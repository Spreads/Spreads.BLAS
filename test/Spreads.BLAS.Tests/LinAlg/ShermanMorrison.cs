                ...
                foreach (var co in columns)
                {
                    ...
                    for (int r = 0; r < rounds; r++)
                    {
                        var statGemm = Benchmark.Run($"Dgemm_{ro}x{co}", count * flopsMult, silent: true);
                        {
                            ...
                            {
                                var data = Data;
                                CBLAS.Dgemm(MatrixLayout.RowMajor, TransCblas.Trans, TransCblas.NoTrans, Columns, Columns, Rows, alpha: 1.0,
                                    data, Columns,
                                    data, Columns,
                                    beta: 0.0,
                                    xxN, Columns);
                        ...
                        var starSyrk = Benchmark.Run($"Dsyrk_{ro}x{co}", count * flopsMult, silent: true);
                        {
                            ...
                            {
                                var data = Data;
                                CBLAS.Dsyrk(MatrixLayout.RowMajor, UpLoCblas.Upper, TransCblas.Trans, Columns, Rows, 1.0,
